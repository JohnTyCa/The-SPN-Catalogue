...
def message(Type, nBlocksToGo = 1):
    ...
    if Type == 'HelloPractice':
        message = ('Welcome to the Practice Experiment. You will see LOTS of patterns and hear beeps. Your task is to decide whether the beeps are low or high pitch:')
    elif Type == 'HelloMain':
...
def responseCollect(soundType,responseScreenVersion):
    event.clearEvents()
    ...
    if responseScreenVersion == 1:
        responseScreen.setText('low          high')
        responseScreen.draw()
        myWin.flip()
        ...
        responseScreen.setText('high          low')
        responseScreen.draw()
        myWin.flip()
        responseKey = event.waitKeys(keyList=['a', 'l']) 
        if responseKey == ['a']:
...
def runBlock(trialbook,Reps):
    ...
    for thisTrial in trials:
        ...
        respCorr, choice = responseCollect(soundType, responseScreenVersion)
