# this is Luke and Ellie's experiment
...
fixationSize = 27
expName='Psymm Sound Task'#from the Builder filename that created this script
expInfo={'participant':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'SOUNDdata/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('SOUNDdata'):
    os.makedirs('SOUNDdata')#if this fails (e.g. permissions) we will get error  


myWin = visual.Window(size = [1920,1080], allowGUI=False, monitor = 'EEG lab participant LCD', units = 'pix', fullscr= True, color=(-1, -1, -1)) # change for new monitor
...
#duration = 0.1
s1 = sound.Sound(200,secs=duration,sampleRate=44100, bits=16)
s2 = sound.Sound(800,secs=duration,sampleRate=44100, bits=16)

def message(Type, nBlocksToGo = 1):
    
    if Type == 'HelloPractice':
        message = ('Welcome to the Practice Experiment. You will see LOTS of patterns and hear beeps. Your task is to decide whether audiovisual mapping is congruent (e.g. low and dark, high and light) or incongruent (low and light, high and dark):')
    elif Type == 'HelloMain':
...
def runBlock(trialbook,Reps):
    ...
    for thisTrial in trials:
        ...
        myWin.flip() 
        parallel.setData(trigger) # THIS IS Better after the win fli. This will make P1 look earlier. 
        core.wait(0.01)
        parallel.setData(0)
        while myClock.getTime() < w:
...
event.waitKeys(keyList = ['g'])
runBlock('SOUNDtrialbook.xlsx',1)
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('SOUNDtrialbook.xlsx',15)
message('Goodbye')
core.wait(2)
myWin.close()
core.quit()
