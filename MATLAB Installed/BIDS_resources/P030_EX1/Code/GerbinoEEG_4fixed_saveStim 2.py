...
myWin = visual.Window(allowGUI=False, units='pix', allowStencil=True, size=(592,422), fullscr=False, monitor = 'EEG lab participant')#creates a window using pixels as units
...
r5 = visual.Rect(myWin, pos=[0, 0], width=100, height=stimH2, fillColor=(-1,-1,1), lineColor=None)
...
smallDot = visual.Circle(myWin, radius=2, fillColor=(-1,-1,-1), lineColor=None, interpolate=True, edges=12)
...
def beads(x, nsteps, symmetry, orientation, p_r, p_y,p_o):
    ...
    diagonal = int(sqrt(2.*stimH2*stimH2))
    diagonal =348
    jigger =8
    rjigger =8
    separation =14
...
#        parallel.setData(0)
        myWin.getMovieFrame()
        myWin.saveMovieFrames('images/'+colorLabel+symmetry+str(orientation)+'_'+str(trialCounter)+'.png')
#        event.waitKeys(keyList=['z'])   #debug
...
myWin.saveMovieFrames('images/image.png')
message('Goodbye')
myWin.close()
core.quit()
