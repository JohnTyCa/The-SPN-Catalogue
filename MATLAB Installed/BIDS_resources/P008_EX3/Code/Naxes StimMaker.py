...
expName='nAxes'#from the Builder filename that created this script
expInfo={'participant':''}
...
hiddenBoundary = visual.Circle(myWin, radius = A, edges = 512, interpolate=True, fillColor = (0.8,0.8,0.8), units = 'pix')
...
for n in range(5):
    for x in range(60):
        pattern(n+1,'degraded',0) #This calls the experiment
        myWin.flip()
        core.wait(0.1)
        myWin.getMovieFrame()
        myWin.flip()
