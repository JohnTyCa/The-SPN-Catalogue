# Solid Poly Randomized Regularity Adam
...
parallel.setData(0)
fixationSize = 10
expName='Solid Poly Randomized Regularity'#from the Builder filename that created this script
expInfo={'participant':'','responseScreenVersion':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'dataRandomizedRegularity/%s_%s_%s' %(expInfo['participant'], expInfo['date'],expInfo['responseScreenVersion'])
if not os.path.isdir('dataRandomizedRegularity'):
    os.makedirs('dataRandomizedRegularity')#if this fails (e.g. permissions) we will get error  
...
s = visual.ImageStim(myWin, mask=None, pos=(300.0, 0.0),size = [1000,1000], ori=0.0, color=(1.0, 1.0, 1.0), colorSpace='rgb', contrast=1.0, opacity=1.0, depth=0, interpolate=True, flipHoriz=False, flipVert=False, texRes=128, name=None, autoLog=None, maskParams=None)
...
def runBlock(trialbook,Reps,blockDuration,nBlocksToGo):
    myWin.flip()
    parallel.setData(108) # One data set identifier right at the start in case we need to do some emergency back check
    core.wait(0.01)
    parallel.setData(0)
    firstImage = 1
