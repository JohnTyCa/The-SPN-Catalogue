...
parallel.setData(0)
expName='passiveColor'#from the Builder filename that created this script
expInfo={'participant':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'passiveColorData/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('passiveColorData'):
    os.makedirs('passiveColorData')#if this fails (e.g. permissions) we will get error  
...
event.waitKeys(keyList = ['g'])
runBlock('trialbookPassiveColor.xlsx',1)
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('trialbookPassiveColor.xlsx',15)
message('Goodbye')
core.wait(2)
myWin.close
core.quit()