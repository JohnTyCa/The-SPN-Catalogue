...
filename= 'memoryShapeData/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('memoryShapeData'):
    os.makedirs('memoryShapeData')#if this fails (e.g. permissions) we will get error  
...
#border = visual.Rect(myWin, height = (radius+dotSize)*2, width = (radius+dotSize)*2,fillColor= 'grey', lineColor = 'black',lineWidth = 1,units= 'pix')
#border2 = visual.Rect(myWin, height = (radius+dotSize)*2, width = (radius+dotSize)*2,fillColor= 'grey', lineColor = 'black',lineWidth = 1,units= 'pix')
...
myWin.saveMovieFrames('s.png') # useful  screen saver system 
message('Goodbye')
core.wait(2)
myWin.close
core.quit()