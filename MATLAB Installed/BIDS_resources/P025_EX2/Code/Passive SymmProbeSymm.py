...
filename= 'PassiveData/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('PassiveData'):
    os.makedirs('PassiveData')#if this fails (e.g. permissions) we will get error  

myWin = visual.Window(size = [1920,1080], allowGUI=False, monitor = 'EEGLAB_3D', units = 'deg', fullscr= True, color=(-1, -1, -1), screen=2,gammaErrorPolicy = 'warn')
myClock = core.Clock()
fixationSize = 0.5
...
def message(Type, nBlocksToGo = 1):
    if Type == 'HelloPractice':
        message = ('Welcome to the Practice Experiment. You will see LOTS of patterns. Your task is to decide whether the second border of the last pattern is red or blue. Please fixate on the red cross')
    elif Type == 'HelloMain':
...
def runBlock(trialbook,Reps):
    ...
    for thisTrial in trials:
        ...
        ser.write(chr(trigger+50).encode()) # for the trigger
        core.wait(0.01)
        ser.write(chr(0).encode())
        ...
        ser.write(chr(trigger+100).encode()) # for the trigger
        core.wait(0.01)
        ser.write(chr(0).encode())
        while myClock.getTime() < w:
            pass # test colored squares on
...
event.waitKeys(keyList = ['g'])
runBlock('trialbookPassive.xlsx',1)
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('trialbookPassive.xlsx',15) #15x8 = 120 trials
message('Goodbye')
core.wait(2)
myWin.close
core.quit()