...
expName='SPN real things Color Disc'#from the Builder filename that created this script
expInfo={'participant':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'dataCol/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('dataCol'):
    os.makedirs('dataCol')#if this fails (e.g. permissions) we will get error  
...
def message(Type, nBlocksToGo = 1):
    if Type == 'HelloPractice':
        message = ('Welcome to the Practice Experiment. You will see LOTS of images. Your task is to decide if they are Color or black and white')
    elif Type == 'HelloMain':
...
def responseCollect(trialType,responseScreenVersion):
    ...
    if responseScreenVersion == 1:
        responseScreen.setText('Colour          Grey')
        responseScreen.draw()
        myWin.flip()
        ...
        if responseKey == ['a']:
            ...
            choice = 'BlackWhite'
            if trialType == 'BlackWhite':
                respCorr = 1
            else:
                respCorr = 0
                
    elif responseScreenVersion == 2:
        responseScreen.setText('Grey          Colour')
        responseScreen.draw()
        myWin.flip()
        responseKey = event.waitKeys(keyList=['a', 'l']) 
        if responseKey == ['l']:
    ...
    for thisTrial in trials:
        ...
        while myClock.getTime() < w:
            if event.getKeys(["escape"]): 
                ...
                event.clearEvents()
                myWin.close
        respCorr, choice = responseCollect(ColourScheme, responseScreenVersion)
...
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('trialbook.xlsx',2) # change to trialbook2 for alternative response screen to image mapping (not very important)
message('Goodbye')
core.wait(2)
myWin.close
core.quit()