...
cd '/Volumes/SPN Catalog/Expanded Catalogue/Project 18/EX3 Ref Unpredictable Ori/Grand Averages'
load timeVector;
load grandAverages;
...
%Oddball = selectedData.Oddball.data;
...
figure('color',[1,1,1])
P = plot(timeVector, [RandRandRand, Predictable, Unpredictable],'LineWidth',LineWidth);
axis([-1000 2000 -10 10]);
set(gca,'YTick',-10:5:10);
set(gca,'XTick',-1000:500:2000,'XMinorTick','on');
set(P,StyleArray,StyleOrder,ColorArray,ColorOrder)
legend({'Random','Repeated Orientations', 'Changing Orientations'},'FontSize',FontSize,'Location','southeast');
...
%GrandAverage ERP differences only
StyleArray = {'LineStyle'};
StyleOrder = {'-','-','--'}';
ColorArray = {'Color'};
ColorOrder = {[1 0 0],[0 1 0],[0 0 0]}';
figure('color',[1,1,1])
P = plot(timeVector, [DiffPredictable, DiffUnpredictable,Zeroline],'LineWidth',LineWidth);
axis([-200 2100 -4 1]);
set(gca,'YTick',-4:1:1,'YTickLabel',[]);
set(gca,'XTick',0:500:2000,'XMinorTick','on','XTickLabel',[]);
set(P,StyleArray,StyleOrder,ColorArray,ColorOrder)
legend({'Repeated Orientations', 'Changing Orientations'},'FontSize',FontSize,'Location','southeast');
