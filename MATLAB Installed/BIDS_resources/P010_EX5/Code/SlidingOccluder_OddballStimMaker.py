...
myWin = visual.Window(allowGUI=False, units='pix', allowStencil=True, size=(800,800),fullscr = False) 
...
def RunBlock(trialbook, Reps):
    ...
    for thisTrial in trials:
        ...
        #parallel.setData(0)
        myWin.getMovieFrame()
        n= "%s%s%s%s" %(regularity,OcclStartPos,color,'T1.png')
...
#        print 'fh', f
        ...
        myWin.getMovieFrame()
        n= "%s%s%s%s" %(regularity,OcclStartPos,color,'T2.png')
        myWin.saveMovieFrames(n)
#        s = myClock.getTime()
