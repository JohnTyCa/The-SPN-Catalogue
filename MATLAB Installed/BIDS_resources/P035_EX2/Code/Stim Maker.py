...
myWin = visual.Window(allowGUI=False, units='pix', allowStencil=False, size=(1280,1024), fullscr=False, monitor = 'testMonitor')#creates a window using pixels as units
...
ResponseScreen=visual.TextStim(myWin, ori=0, text='Prompt', pos=[0, 50], height=34, color='Black', units= 'pix')
...
#    print coord1
    ...
    if orientation ==-1:
        ...
        occluder.setVertices(coord1)
        stimL.setVertices(fooL)
        stimR.setVertices(fooR)    
...
def runBlock(filename):
    ...
    if filename=='data/practice':
        trials=data.TrialHandler(nReps=1, method='random', trialList=data.importConditions('trialbookPractice.xlsx')) #
    else:
        trials=data.TrialHandler(nReps=1, method='sequential', trialList=data.importConditions('trialbook.xlsx')) #
