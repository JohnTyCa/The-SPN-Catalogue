...
def responseCollect(ResponseScreenLimit, Prompt, correctKey, screenLag):
    ...
    responseKey = event.waitKeys(keyList=['a','d','j','l','escape'], maxWait = ResponseScreenLimit)
...
def runBlock(filename):
    ...
    if filename=='data/practice':
        trials=data.TrialHandler(nReps=1, method='random', trialList=data.importConditions('trialbook2.xlsx')) #('trialbookPractice.xlsx')) 
    else:
        trials=data.TrialHandler(nReps=10, method='random', trialList=data.importConditions('trialbook2.xlsx')) #
