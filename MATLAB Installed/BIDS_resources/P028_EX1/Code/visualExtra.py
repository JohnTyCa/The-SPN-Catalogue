...
# return whether the vertices are clockwise (1) or counterclockwise (-1)
def clockwise(poly):
    ...
    return sign(temp1 - temp2)
    
#Return a list of inner (convex) angles and a second list of concave (360 - outer) angles
#original code here: http://stackoverflow.com/questions/20252845/best-algorithm-for-detecting-interior-and-exterior-angles-of-an-arbitrary-shape
def computeAngles(poly):
    ...
    if clockwise(poly1) == -1:   # making sure the vertices are entered clockwise 
        poly.reverse()
...
#        print('Angle', math.degrees(angle(x1, y1, x2, y2) ))
        if cross_sign(x1, y1, x2, y2):
            convex.append(math.degrees(angle(x1, y1, x2, y2) ))
            both.append(math.degrees(angle(x1, y1, x2, y2) ))
        else:
            concave.append(math.degrees(angle(x1, y1, x2, y2) ))            
            both.append(-math.degrees(angle(x1, y1, x2, y2) ))
...
#a =[[-200, -200], [-200, 200], [200, 200],[0,0],[200,-200],[-200,-200]]
#x,e =computeAngles(a)
#print (x,e)
#print(a)
...
#
#v = rotateVertex(vertex=[10,20], a=180, pos=[10,0])
#print v
