...
myWin = visual.Window(size = [600,300], allowGUI=False, monitor = 'testMonitor', units = 'pix', fullscr= False)
...
fixation2 =visual.Line(myWin, start=[10,0], end=[-10,0], lineColor='black')
...
def mainLoop(n, book):
    ...
    trialCounter = 0
    nBlocks = 12
    trials=data.TrialHandler(nReps=n, method='sequential', trialList=data.importConditions(book)) 
...
#        parallel.setData(trigger)
        myWin.flip()
        n= "%s%s%s%s" %(cond,symmetry,co,'.png')
        myWin.getMovieFrame()
