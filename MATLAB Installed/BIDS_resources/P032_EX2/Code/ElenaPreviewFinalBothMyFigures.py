...
myWin = visual.Window(allowGUI=False, color=[0,0,0], units='pix', size=(800,800), allowStencil=True, fullscr= False, screen = 1)
...
rdp0 = visual.GratingStim(myWin, tex=myTexture, size=[512,512],color= 'black')
myTexture = numpy.random.randint(0,2,(256,256))-0.8
...
def message(type, nBlocksToGo = 1):
    ...
    instructions.setPos([dispUp, -upshift/2])
    instructions.draw()
    instructions.setPos([dispDown, upshift/2])
    instructions.draw()
    myWin.flip()
...
def drawStim(sten, cond): #stencilL, stencilR,
    ...
    # this is the preview       
        if cond == 'figure'or cond == 'ground':  # cond in ['figure', 'ground']  
            ...
            rdp0.setPos([20, 45])
            rdp0.draw()
#            rdp0.setPos([rlShift, 0])
#            rdp0.draw()
#        
        if cond == 'figure':
            sten.enabled = True
            sten.inverted = False
            rdp1.setPos([-rlShift, dispUp])
            sten.setPos ([-rlShift,dispUp])
            rdp1.draw()
#            rdp1.setPos([rlShift,dispDown])
#            sten.setPos ([rlShift, dispDown])
#            rdp1.draw()
...
#            rdp1.draw()  
        ...
        fixation.setPos([-rlShift,dispUp])
        fixation.draw()
#        fixation.setPos([ rlShift,dispDown])
#        fixation.draw()
...
#    responseScreen.draw()
    myWin.flip()
#    myWin.getMovieFrame()
#    myWin.saveMovieFrames("images/"+"responsescreen.png")
...
def mainLoop(trialbook, reps, practiceExp): # define Reps and set at bottom how many blocks you want  
    ...
    for thisTrial in trials:
        ...
        myWin.flip()
        myWin.getMovieFrame()
        myWin.saveMovieFrames("images/"+figureground+"preview.png")
#        #myWin.flip()
...
# this is the stimulus
        ...
        myWin.flip()
        myWin.getMovieFrame()
        myWin.saveMovieFrames("images/"+figureground+symmetry+".png")
#        print (cond, symmetry)
