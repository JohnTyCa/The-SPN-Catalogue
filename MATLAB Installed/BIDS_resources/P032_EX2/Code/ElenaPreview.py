...
verticalSize = 1080
disp = 8
myWin = visual.Window(allowGUI=False, units='pix', size=(800,verticalSize), allowStencil=True, viewScale=[1.0, 0.5], fullscr= False, screen = 0)
...
myTexture = numpy.random.randint(0,2,(256,256))-0.5
rdp1 = visual.GratingStim(myWin, tex=myTexture, size=[512,512], color='green')
...
def mainLoop():
    ...
    for thisTrial in trials:
        ...
        v = makePattern(0, 0, 'circle', nV, 140, 0)
        if symmetry == 'sym':
            v =  makePattern(0, 0, 'circle', nV, 140, 2)
        #continue
        ...
        stencilR = visual.Aperture(myWin, units="pix", shape= vertices, size = 1)
        myWin.flip()
...
# this is the preview
        if cond == 'figure'or 'ground':
            stencilR.enabled = False
            rdp0.setPos([disp, -upshift])
            rdp0.draw()
        ...
        if cond == 'figure':
            ...
            stencilR.setPos ([-disp, -upshift])
            rdp1.draw()
            rdp1.setPos([0, upshift])
            stencilR.setPos ([0, upshift])
            rdp1.draw()
...
#        newlist.append(topL)
#        newlist.append(topR)
#        newlist.append(bottomR)
#        newlist.append(bottomL)
#        temp.vertices = list(newlist)
#        temp.draw()
