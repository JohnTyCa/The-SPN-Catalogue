...
verticalSize = 1080
dispRightEye = 0# disparity in the right eye, if negative shift is to the left
dispLeftEye =  0
...
barWidth = 14
dotBrightness = 0.2
upshift = 0 #verticalSize/2 #upshift value
baselineDuration = 1.5

upshift =  0 # 266 #upshift value
lrShift = 0 # -266 
baselineDuration = 1.5

myWin = visual.Window(allowGUI=False,  units='pix', size=(512,512), allowStencil=True, fullscr= False, screen = 1, color = 'white')
...
myTexture = numpy.random.randint(0,2,(256,256))-0.8
rdp1 = visual.GratingStim(myWin, tex=myTexture, size=[512,512], color= 'black')
...
myWin.setPerspectiveView(applyTransform=True, clearDepth=True)
...
def message(type, nBlocksToGo = 1):
    ...
    instructions.setPos([dispRightEye + lrShift, -upshift/2])
    instructions.draw()
    instructions.setPos([dispLeftEye - lrShift, upshift/2])
    instructions.draw()
    myWin.flip()
...
def drawStim(sten, cond): #stencilL, stencilR,
    ...
    # this is the preview       
        if cond == 'figure'or cond == 'ground':  # cond in ['figure', 'ground']  
            sten.enabled = False
#            rdp0.setPos([0 + lrShift, -upshift])    # right eye 
#            rdp0.draw()
#            rdp0.setPos([0 - lrShift, upshift])     # left eye
#            rdp0.draw()
        
        if cond == 'figure':
            sten.enabled = True
            sten.inverted = False
#            rdp1.setPos([dispRightEye + lrShift, -upshift])  # right eye
#            sten.setPos ([dispRightEye + lrShift, -upshift])
#            rdp1.draw()
#            rdp1.setPos([dispLeftEye - lrShift, upshift])    # left eye
#            sten.setPos ([dispLeftEye - lrShift, upshift])
#            rdp1.draw()
            
        elif cond == 'ground': 
            sten.enabled = True
            sten.inverted = True
#            rdp1.setPos([dispRightEye + lrShift, -upshift])
#            sten.setPos ([dispRightEye + lrShift, -upshift])
#            rdp1.draw()
#            rdp1.setPos([dispLeftEye - lrShift, upshift])
#            sten.setPos ([dispLeftEye - lrShift, upshift])
#            rdp1.draw()  
        sten.enabled = False
       
#        fixation.setPos([dispRightEye + lrShift, -upshift])
#        fixation.draw()
#        fixation.setPos([dispLeftEye - lrShift, upshift])
#        fixation.draw()
...
def responseCollect(trialType, screen, practiceExp):
    ...
    responseScreen.setPos([0 + lrShift, -upshift/2])
    responseScreen.draw()
    responseScreen.setPos([0 - lrShift, upshift/2])
    responseScreen.draw()
    myWin.flip()
...
def mainLoop(trialbook, reps, practiceExp): # define Reps and set at bottom how many blocks you want  
    ...
    for thisTrial in trials:
        ...
        myWin.flip()
        myWin.getMovieFrame()
        myWin.saveMovieFrames("images/"+figureground+"previeww.png")
        #myWin.flip()
...
# this is the stimulus
        ...
        myWin.flip()
        myWin.getMovieFrame()
        myWin.saveMovieFrames("images/"+figureground+symmetry+"w.png")
#        print (cond, symmetry)
