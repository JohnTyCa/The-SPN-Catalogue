...
# this is the same as REF and IDENT, but without the seed, background set to black and square at 600 instead of 800, grey instead of black edge, no big red fixation dot
...
myWin = visual.Window(size = [600,600], allowGUI=False, monitor = 'testMonitor', units = 'pix', fullscr= False, color=(-1, -1, -1))
...
background = visual.Circle(myWin, radius = A+(DS/2), edges = 512,  interpolate=True, lineColor = 'grey', fillColor = 'grey', lineWidth  = 0, units = 'pix') # I've done quite a bit of hacking because there are strange constraints on lineWidth here
...
#    pattern(folds,'degraded',0) #This calls the experiment
#    myWin.flip()
#    core.wait(duration)
#    myWin.getMovieFrame()
#    myWin.flip()
