...
cd '/Users/alexismakin/Desktop/Alexis Work/Symmetry EEG/SPN effect size and power/Project Folders V8/Project 13/EX1 Reg Task/Grand Averages';
load timeVector;
load grandAverages; 
load ERPerror;
%CONDITIONS
...
% used for figure in Box 1
close all
figure('color',[1,1,1])
StyleArray = {'LineStyle'};
StyleOrder = {'-','-','-','-','-'}';
ColorArray = {'Color'};
ColorOrder = {[0.8 0 0.2],[0.6 0 0.4], [0.4 0 0.6],[0.2 0 0.8],[0 0 1]}';
figure('color',[1,1,1]);

rectangle('position',[low, -5,high-low,6],'FaceColor',[1 1 0 0.1],'EdgeColor','black'); 
patch([timeVector,fliplr(timeVector)],[NegCISPN20',fliplr(PosCISPN20')],ColorOrder{1},'EdgeColor','none');
hold on
plot(timeVector, Diff20,'color',[0.7 0 0.1],'LineWidth',LineWidth);
hold on
patch([timeVector,fliplr(timeVector)],[NegCISPN40',fliplr(PosCISPN40')],ColorOrder{2},'EdgeColor','none');
hold on
plot(timeVector, Diff40,'color',[0.5 0 0.3],'LineWidth',LineWidth);
hold on
patch([timeVector,fliplr(timeVector)],[NegCISPN60',fliplr(PosCISPN60')],ColorOrder{3},'EdgeColor','none');
hold on
plot(timeVector, Diff60,'color',[0.3 0 0.5],'LineWidth',LineWidth);
hold on
patch([timeVector,fliplr(timeVector)],[NegCISPN80',fliplr(PosCISPN80')],ColorOrder{4},'EdgeColor','none');
hold on
plot(timeVector, Diff80,'color',[0.1 0 0.7],'LineWidth',LineWidth);
hold on
patch([timeVector,fliplr(timeVector)],[NegCISPN100',fliplr(PosCISPN100')],ColorOrder{5},'EdgeColor','none');
hold on
plot(timeVector, Diff100,'color',[0 0 0.9],'LineWidth',LineWidth);
hold on

legend({'20', '40','60', '80','100'},'FontSize',FontSize,'Location','southwest');
%P = plot(timeVector, Diff100,'color','black','LineWidth',LineWidth);
%set(P,StyleArray,StyleOrder,ColorArray,ColorOrder)
