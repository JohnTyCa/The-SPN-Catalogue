...
#parallel.setData(0)
fixationSize = 40
expName='Psymm Reg Task'#from the Builder filename that created this script
expInfo={'participant':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'REGdata/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('REGdata'):
    os.makedirs('REGdata')#if this fails (e.g. permissions) we will get error  
...
def responseCollect(trialType,responseScreenVersion):
    ...
    if responseScreenVersion == 1:
        ...
        if responseKey == ['a']:
            choice = 'Some Regularity'
            if trialType == 0:
                respCorr = 0
            else:
                respCorr = 1
        elif responseKey == ['l']:
            choice = 'No Regularity'
            if trialType == 0:
                respCorr = 1
            else:
                respCorr = 0
...
event.waitKeys(keyList = ['g'])
runBlock('REGtrialbook.xlsx',1)
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('REGtrialbook.xlsx',30)
message('Goodbye')
core.wait(2)
myWin.close
core.quit()
