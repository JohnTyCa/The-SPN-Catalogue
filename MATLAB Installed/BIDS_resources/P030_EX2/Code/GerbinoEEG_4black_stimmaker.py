...
myWin = visual.Window(allowGUI=False, units='pix', allowStencil=True, size=(592,422), fullscr=False)#creates a window using pixels as units
...
def runBlock(filename):
    ...
    if filename=='data/practice':
        ...
        numrep=10
    trials=data.TrialHandler(nReps=numrep, method='sequential', trialList=data.importConditions('trialbookBlack.xlsx')) #
