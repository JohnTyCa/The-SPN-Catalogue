...
duration = 0.1 # how many seconds do you want each shape to be presented? This is less than in the Psychophysiology paper, because we never explore the last second anyway. 
...
maxCol = 0.1# whats the highest intensity? -1 to 1
screenLag = 0.1
baseline = 0.1
...
def runBlock(trialbook):
    ...
    myClock = core.Clock()
    trials=data.TrialHandler(nReps=1, method='sequential', trialList=data.importConditions(trialbook))
    ...
    for thisTrial in trials:
        ...
        myWin.getMovieFrame()
        n= "%s%s%s%s" %(label,orientation,counter,'.png')
