# Solid Poly Blocked Luminance John
...
expName='Solid Poly Blocked Luminance'#from the Builder filename that created this script
expInfo={'participant':'','order':'','responseScreenVersion':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'dataBlockedLuminance/%s_%s_%s_%s' %(expInfo['participant'], expInfo['date'],expInfo['order'],expInfo['responseScreenVersion'])
if not os.path.isdir('dataBlockedLuminance'):
    os.makedirs('dataBlockedLuminance')#if this fails (e.g. permissions) we will get error  
...
def runBlock(trialbook,Reps,trialSequence,blockDuration,nBlocksToGoEx):
    myWin.flip() 
    parallel.setData(105) # One data set identifier right at the start in case we need to do some emergency back check
    core.wait(0.01)
    parallel.setData(0)
    firstImage = 33
