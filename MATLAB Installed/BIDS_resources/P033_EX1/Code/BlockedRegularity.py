# Solid Poly Blocked Regularity John
...
expName='Solid Poly Blocked Regularity'#from the Builder filename that created this script
expInfo={'participant':'','order':'','responseScreenVersion':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'dataBlockedRegularity/%s_%s_%s_%s' %(expInfo['participant'], expInfo['date'],expInfo['order'],expInfo['responseScreenVersion'])
if not os.path.isdir('dataBlockedRegularity'):
    os.makedirs('dataBlockedRegularity')#if this fails (e.g. permissions) we will get error  
...
exampleFrame =visual.ImageStim(myWin, mask=None, pos=(0.0, -200.0), ori=0.0, color=(1.0, 1.0, 1.0), colorSpace='rgb', contrast=1.0, opacity=1.0, depth=0, interpolate=True, flipHoriz=False, flipVert=False, texRes=128, name=None, autoLog=None, maskParams=None)
responseScreen=visual.TextStim(myWin, ori=0, pos=[0, 0], height=30, color='blue', colorSpace='rgb', units= 'pix')
Instructions=visual.TextStim(myWin, ori=0, pos=[0, 200], height=30, color='white', colorSpace='rgb', units= 'pix')
...
def message(Type, nBlocksToGo=1):
    if Type == 'HelloPractice':
        message = ('Welcome to the Practice Experiment. You will see LOTS of patterns. Your task is to classify them as random or symmetry')
    elif Type == 'HelloMain':
        ...
        message = '%d Blocks to go. The next block has the same squares again. Wait for Experimenter to check electrodes'%nBlocksToGo
    elif Type == 'Hello00':
        message = '%d Blocks to go.In the next block, the square stimuli will all look like this. Wait for Experimenter to check electrodes'%nBlocksToGo
        exampleFrame.setImage('00frame.png')
        exampleFrame.draw()
    elif Type == 'Hello6015':
        message = '%d Blocks to go.In the next block, the square stimuli will all look like this. Wait for Experimenter to check electrodes'%nBlocksToGo
        exampleFrame.setImage('6015frame.png')
        exampleFrame.draw()
    elif Type == 'Hello60-15':
        message = '%d Blocks to go.In the next block, the square stimuli will all look like this. Wait for Experimenter to check electrodes'%nBlocksToGo
        exampleFrame.setImage('60-15frame.png')
        exampleFrame.draw()
    elif Type == 'Hello-6015':
        message = '%d Blocks to go.In the next block, the square stimuli will all look like this. Wait for Experimenter to check electrodes'%nBlocksToGo
        exampleFrame.setImage('-6015frame.png')
        exampleFrame.draw()
    elif Type == 'Hello-60-15':
        message = '%d Blocks to go.In the next block, the square stimuli will all look like this. Wait for Experimenter to check electrodes'%nBlocksToGo
        exampleFrame.setImage('-60-15frame.png')
        exampleFrame.draw()
    elif Type == 'Goodbye':
...
def runBlock(trialbook,Reps,trialSequence,blockDuration,nBlocksToGoEx):
    parallel.setData(106) # One data set identifier right at the start in case we need to do some emergency back check
    core.wait(0.01)
    parallel.setData(0)
    firstImage = 33
    nImages = 32 # can use first half of the images in folders for the randomized block, or vice versa. This needs to be counterbalanced
    ...
    trialCounter = 0
    nBlocksToGo = nBlocksToGoEx
    
 
   
    trials=data.TrialHandler(nReps=Reps, method=trialSequence, trialList=data.importConditions(trialbook))
    for thisTrial in trials:
        ...
        respCorr, choice = responseCollectReg(Reg, responseScreenVersion)
...
event.waitKeys(keyList = ['g'])
runBlock('trialbook.xlsx',1,'random',blockDuration,2)
message('HelloMain')
        
        
if expInfo['order'] == '1':
    trialBookOrder = [1,2,3,4,5]
elif expInfo['order'] == '2':
    trialBookOrder = [2,3,4,5,1]
elif expInfo['order'] == '3':
    trialBookOrder = [1,4,5,2,3]
elif expInfo['order'] == '4':
    trialBookOrder = [4,5,2,3,1]
    
    
    
#elif expInfo['order'] == '5':
#    trialBookOrder = [1,3,2,5,4]
#elif expInfo['order'] == '6':
#    trialBookOrder = [3,2,5,4,1]
#elif expInfo['order'] == '7':
#    trialBookOrder = [1,5,4,3,2]
#elif expInfo['order'] == '8':
#    trialBookOrder = [5,4,3,2,1]


for tBooks in range(len(trialBookOrder)):
    tBook = trialBookOrder[tBooks]


    if tBook == 1:
        message('Hello00',nBlocksToGo)
        event.waitKeys(keyList = ['g'])
        nBlocksToGo = runBlock('00.xlsx',1,'random',blockDuration,nBlocksToGo) # 8 blocks of 32 trials
    elif tBook == 2:
        message('Hello6015',nBlocksToGo)
        event.waitKeys(keyList = ['g'])
        nBlocksToGo=runBlock('6015.xlsx',1,'random',blockDuration,nBlocksToGo) # 8 blocks of 32 trials
    elif tBook == 3:
        message('Hello60-15',nBlocksToGo)
        event.waitKeys(keyList = ['g'])
        nBlocksToGo=runBlock('60-15.xlsx',1,'random',blockDuration,nBlocksToGo) # 8 blocks of 32 trials
    elif tBook == 4:
        message('Hello-6015',nBlocksToGo)
        event.waitKeys(keyList = ['g'])
        nBlocksToGo=runBlock('-6015.xlsx',1,'random',blockDuration,nBlocksToGo) # 8 blocks of 32 trials
    elif tBook == 5:
        message('Hello-60-15',nBlocksToGo)
        event.waitKeys(keyList = ['g'])
        nBlocksToGo=runBlock('-60-15.xlsx',1,'random',blockDuration,nBlocksToGo) # 8 blocks of 32 trials
