...
#expName='CongruenceEEG'#from the Builder filename that created this script
#expInfo={'participant':'', 'session':'','order':''}
...
myWin = visual.Window(allowGUI=False, size=[screenWidth,screenHeight], units='pix', fullscr= False)
...
#        core.wait(ITI)
        ...
        myWin.flip()
        myWin.getMovieFrame()
        n= "%s%s%s%s" %(Valence,Regularity,Counter,'.png')
        ...
        while myClock.getTime() < t:
            if event.getKeys(["escape"]): 
                    ...
                    myWin.close
                    core.quit()
#        responseScreen(respScreenType,Valence,Regularity,0)
