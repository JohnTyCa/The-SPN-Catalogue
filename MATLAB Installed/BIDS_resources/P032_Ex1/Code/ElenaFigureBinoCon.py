...
expName='ElenaBinoCon'#from the Builder filename that created this script
...
#            myTexture0 = numpy.random.randint(0,2,(128,128))- dotBrightnessCon
            myTexture0 = numpy.random.randint(0,2,(128,128)) *2 - 1           
            rdp0.tex = myTexture0
            sten.enabled = False
            rdp0.setPos([0, -upshift])    # right eye 
            rdp0.draw()
        ...
        if cond == 'binocular'or cond == 'contrast':  
            sten.enabled = True
            sten.inverted = False
...
def mainLoop(trialbook, reps, practiceExp): # define Reps and set at bottom how many blocks you want  
    ...
    for thisTrial in trials:
        ...
        cond = 'contrast'  # debugging
        print (binoCon, symmetry, screen)
...
event.waitKeys(keyList = ['g'])
mainLoop('bookBinoCon.xlsx', 2, "practice") #32 practice trials
message('HelloMain')
event.waitKeys(keyList = ['g'])
mainLoop('bookBinoCon.xlsx', 30, "exp") #30 x 16 = 480 trials    
message('Goodbye')
core.wait(2)
ser.close()
myWin.close()
