...
expName='CompletionOccluder_Pointer'#from the Builder filename that created this script
expInfo={'N':0,'order':1}
...
pointertopvert= [[0,40],[40,-40],[-40,-40]]
pointertop=visual.ShapeStim(myWin, pos=[0,0], vertices=pointertopvert, lineColor=r, lineWidth=2, fillColor=None, units= 'pix')
pointerbottomvert= [[0,-40],[40,40],[-40,40]]
pointerbottom=visual.ShapeStim(myWin, pos=[0,0], vertices=pointerbottomvert, lineColor=r,lineWidth=2, fillColor=None, units= 'pix')
#
...
def message(Type, nBlocksToGo = 1):
    if Type == 'HelloPractice':
        message = ('Welcome to the Practice Experiment.'
                    'In each trial you will see one red triangle in the center of the screen, pointing upwards or downwards. '
                    '\n \n''This is presented for a very short time.'
                    '\n''one of the rectangle bars will move towards the center and cover the stimulus for a short interval.'
                    '\n''After that, the bar will slide back towards the original position and stop halfway, so that half of the triangle underneath will be shown'
                    '\n''your task is to report whether' 
                    '\n''the TRIANGLE is POINTING TOWARDS the SAME or a DIFFERENT direction as before. '
...
#        print 'pA', pA
        if saveImage:
            myWin.getMovieFrame()
            myWin.saveMovieFrames("savedimages/polyA"+imageID_1+pointer+".png")
...
#        ma= myClock.getTime()
        ...
        #we don't draw a polygon afterwards
...
##        print 'whole thing', wt
        if saveImage:
            myWin.getMovieFrame()
            myWin.saveMovieFrames("savedimages/polyA"+imageID_2+pointer+".png")
...
#
if expInfo['order']== 1: 
    trialbook='COpointer_trialbook1.xlsx'
elif expInfo['order']== 2: 
    trialbook='COpointer_trialbook2.xlsx'
elif expInfo['order']== 3: 
    trialbook='COpointer_trialbook3.xlsx'
elif expInfo['order']== 4: 
    trialbook='COpointer_trialbook2.xlsx'
elif expInfo['order']== 5: 
    trialbook='COpointer_trialbook3.xlsx'
elif expInfo['order']== 6: 
    trialbook='COpointer_trialbook6.xlsx' 
elif expInfo['order']== 7: 
    trialbook='COpointer_trialbook7.xlsx'
elif expInfo['order']== 8: 
    trialbook='COpointer_trialbook8.xlsx'
