...
def generateCoords(n=8, pos=[0,0], minDist=1, radius=100, symmetry='r', start=0, stop=360):
def generateCoordsPolar(n=10, pos=[0,0], minDist=1, gapAxis=2, radius=100, start=0, stop=360):
def generateSymmetry(symmetry='no', axes=0, ndots=32, radius=300):
...
#returns the coords of a poly v after rotation by angle a (in degrees) 
def rotateVertices(vertices=[[0,0],[0,0]], pos=[0,0], a=90):
...
def generateCoordsPolar(n=10, pos=[0,0], minDist=1, gapAxis=2, radius=100, start=0, stop=360):
    ...
    for i in range(n):
        ...
        while farEnough == False:
            r = minDist + int((radius - minDist)*sqrt( random.rand() ))
#                r= random.randint(2, radius)
            a =random.randint(start+gapAxis, stop-gapAxis)
...
def overlapCoords(vertices1=[[0,0],[0,0]], vertices2=[[0,0],[0,0]], minDist=1):
    ...
    vc1 = [polar2cartesian(i[0],i[1]) for i in vcopy1]  # vertices as cartesian coords
    vc2 = [polar2cartesian(i[0],i[1]) for i in vcopy2]  # vertices as cartesian coords
...
#for i in range(20):
#    target = generateSymmetry(symmetry='r', axes=4, ndots=32, radius=200, minDist=5.3, gapAxis=3)
#    print len(target)
