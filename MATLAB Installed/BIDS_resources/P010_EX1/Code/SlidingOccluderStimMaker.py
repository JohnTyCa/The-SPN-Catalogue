...
myWinColor= [-.3,-.3,-.3]
#
myWin = visual.Window(allowGUI=False, units='pix', allowStencil=True, color= myWinColor, size=(800,800),fullscr = False) #size=(800,600)) # , monitor = 'EEG lab participant')#creates a window using pixels as units
myClock = core.Clock()
...
def RunBlock(trialbook, Reps):
    ...
    FirstHalfduration=2#.5
    SecondHalfduration = 2
    shiftPos= 181
    speedOccl= 181
    ...
    for thisTrial in trials:
        ...
        myWin.flip()
        myWin.getMovieFrame()
        n= "%s%s%s" %(regularity,OcclStartPos,'T1.png')
...
##        print 'fh', fh
        ...
        myWin.getMovieFrame()
        n= "%s%s%s" %(regularity,OcclStartPos,'T2.png')
        myWin.saveMovieFrames(n)
...
RunBlock(trialbook, 1)
#message('HelloMain')
#event.waitKeys('Space')
#RunBlock(trialbook, 30) # (240 trials: 2 conditions (4subconditions): 120 reps) . 
#message('Goodbye')
#event.waitKeys('Space')
