...
density =10 # in percent, how many positions get filled
...
e = visual.Circle(myWin, radius = DS/2.5, interpolate=True, fillColor= 'black', lineColor = 'black',lineWidth  = 0)
...
def pattern(folds,reg,pRef):
    ...
    background2.draw()
    memCoords = 'NA'
    tCoords = setup(density,folds,reg,pRef,memCoords)
    memCoords = tCoords
    for segment in range(folds): 
        ...
        for p in range ((DSSW/2)*DSSW):
            ...
            if tCoords[p,5] ==1: # if its an element
                 ...
                 yPos = tCoords[p,2] 
                 xPos = tCoords[p,3] # These were switched to get flip horizontal
                 xPos, yPos = rotateCoords(xPos,yPos,radians((360/folds)*segment)) # angle is in radians
                 if elementShape == 'circle':
