...
figure('color',[1,1,1])
P = plot(timeVector, [RandRandRand, Rep],'LineWidth',LineWidth);
axis([-200 2100 -10 10]);
...
% hold on
% plot(timeVector,RepIndividuals,'LineWidth',0.05,'Color',ColorOrder{2},'LineStyle', ':');
% hold on 
% uistack(P,'top')
% legend('off')
...
ColorOrder = {[1 0 0],[0 0 1]}';
figure('color',[1,1,1])
P = plot(timeVector, [RandRandRand, Change],'LineWidth',LineWidth);
axis([-200 2100 -10 10]);
set(gca,'YTick',-10:5:10);
set(gca,'XTick',-0:500:2000);
set(P,StyleArray,StyleOrder,ColorArray,ColorOrder)
...
% plot(timeVector, RandRandRandIndividuals,'LineWidth',0.05,'Color',ColorOrder{1},'LineStyle', ':');
% hold on
% plot(timeVector,ChangeIndividuals,'LineWidth',0.05,'Color',ColorOrder{2},'LineStyle', ':');
% hold on 
% uistack(P,'top')
% legend('off')
...
MSPNRep = mean(DiffRep(ii));
MSPNChange = mean(DiffChange(ii));


SPNcheck = [MSPNRep,MSPNChange]
save('SPNcheck','SPNcheck');

CIRepSPN = ERPerror.RepSPN.CI;
CIChangeSPN = ERPerror.ChangeSPN.CI;


if strcmp(smoothOn, 'on') == 1;
    CIRepSPN = smooth(CIRepSPN,smoothfactor,'moving');
    CIChangeSPN = smooth(CIChangeSPN,smoothfactor,'moving');
    CIRandRandRefSPN = smooth(RandRandRefSPN,smoothfactor,'moving');
end

PosCIRepSPN= DiffRep+CIRepSPN;
PosCIChangeSPN= DiffChange+CIChangeSPN;


NegCIRepSPN= DiffRep-CIRepSPN;
NegCIChangeSPN= DiffChange-CIChangeSPN;


figure('color',[1,1,1])
themeCol = 'blue';
rectangle('position',[low, -5,high-low,6],'FaceColor',[1 1 0 0.1],'EdgeColor','black'); 
patch([timeVector,fliplr(timeVector)],[NegCIRepSPN',fliplr(PosCIRepSPN')],themeCol,'EdgeColor','none');
hold on
plot(timeVector,DiffRep,themeCol,'LineWidth',LineWidth);
hold on
plot(timeVector, Zeroline,'Color',[1 0 0],'LineWidth',LineWidth,'LineStyle','-');
axis([-200 2100 -5 1]);
set(gca,'YTick',-5:1:1);
set(gca,'XTick',-0:500:2000,'XMinorTick','on');
yline(MSPNRep,themeCol,'LineWidth',LineWidth);
xlabel('Time from stimulus onset (ms)');
...
themeCol = 'blue';
rectangle('position',[low, -5,high-low,6],'FaceColor',[1 1 0 0.1],'EdgeColor','black'); 
patch([timeVector,fliplr(timeVector)],[NegCIChangeSPN',fliplr(PosCIChangeSPN')],themeCol,'EdgeColor','none');
hold on
plot(timeVector,DiffChange,themeCol,'LineWidth',LineWidth);
hold on
plot(timeVector, Zeroline,'Color',[1 0 0],'LineWidth',LineWidth,'LineStyle','-');
axis([-200 2100 -5 1]);
set(gca,'YTick',-5:1:1);
set(gca,'XTick',-0:500:2000,'XMinorTick','on');
yline(MSPNChange,themeCol,'LineWidth',LineWidth);
xlabel('Time from stimulus onset (ms)');
ylabel('SPN amplitude (microvolts)');
grid('on')
legend('off')
