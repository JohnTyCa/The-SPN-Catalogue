...
def runBlock(trialbook,Reps):
    ...
    VList = pictureShuffle(180,'Vertical')  # this is a shuffled list of Reflection image names
    ...
    for thisTrial in trials:
        ...
        if S1 == 'Horizontal':
            HCounter = HCounter+1
            image1 = HList[HCounter]
        if S2 == 'Horizontal':
            if identical < 1:
                HCounter = HCounter+1
            image2 = HList[HCounter]
        if S3 == 'Horizontal':
            if identical < 1:
                HCounter = HCounter+1
            image3 = HList[HCounter]
...
event.waitKeys(keyList = ['g'])
runBlock('trialbook V and H.xlsx',1)
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('trialbook V and H.xlsx',15)
message('Goodbye')
core.wait(2)
myWin.close
core.quit()
