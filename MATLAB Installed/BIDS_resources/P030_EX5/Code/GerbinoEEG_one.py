...
expInfo={'participant':'', 'subnumber':1}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
seed(expInfo['subnumber'])
...
#        parallel.setData(trigger)
        ...
        imagename = 'images1/'+'p%s'%(trialCounter) +symmetry + colore + str(orientation) + '.png'
...
#            event.clearEvents()
#            myWin.close
#        presentation = myClock.getTime() +.5
...
#        key = event.waitKeys(keyList=["a"])
        print symmetry, trigger, trialCounter
