...
expName='GerbinoEEG_PNG1'#from the Builder filename that created this script
...
def message(Type):
    if Type == 'HelloExperiment':
        ...
        instruction.setText ('%d blocks to go: Wait for experimenter to check electrodes!' %nBlocksToGo)
    instruction.draw()
...
def runBlock(filename):
    ...
    elif filename =='data/experiment1':
        numrep=1
...
#        event.waitKeys(keyList=["a"])
        ...
        if filename=='data/practice1':
            stim1.setImage('images1/'+'p%s' % (TrialNumberT) +symmetryT + coloreT+str(orientationT)+'.png')
        elif filename=='data/experiment1':
            stim1.setImage('images1/'+'%s' % (TrialNumberT) +symmetryT + coloreT+str(orientationT)+'.png')
...
#        print symmetry, orientation, colore
...
runBlock('data/practice1')

message('HelloTask')
event.waitKeys(keyList=['space'])
runBlock('data/experiment1')
message('Goodbye')
myWin.close()
core.quit()
