...
expName='checks'#from the Builder filename that created this script
expInfo={'participant':''}
dlg=gui.DlgFromDict(dictionary=expInfo,title=expName)
if dlg.OK==False: core.quit() #user pressed cancel
expInfo['date']=data.getDateStr()#add a simple timestamp
expInfo['expName']=expName
filename= 'dataChecks/%s_%s' %(expInfo['participant'], expInfo['date'])
if not os.path.isdir('dataChecks'):
    os.makedirs('dataChecks')#if this fails (e.g. permissions) we will get error  
...
DS = 25 # DS = 'Downsampling' i.e. how many pixels between the elements
...
fixation = visual.Circle(myWin, radius = DS/2, interpolate=True, lineWidth = 0,fillColor= 'red', lineColor = 'black')
...
e = visual.Rect(myWin, size = [DS*2,DS*2], interpolate=True, fillColor= 'blue', lineColor = 'blue',lineWidth = 0)
...
def message(Type, nBlocksToGo = 1):
    ...
    Instructions2=visual.TextStim(myWin, ori=10, text='EXPERIMENTER! REMEMBER THE CHIN REST! and participant 140 cm from screen', pos=[0, -200], height=30, color='red', colorSpace='rgb', units= 'pix')
    Instructions.draw()
    Instructions2.draw()
    myWin.flip()
...
def pattern(folds,reg,pRef,colorCategory,antiS,density):
    ...
    colLevel = 0.5
    if colorCategory == 'BW':
        color1 = [-bwLevel,-bwLevel,-bwLevel]
        color2 = [bwLevel,bwLevel,bwLevel]
    elif colorCategory == 'Col':
        color1 = [-bwLevel,-colLevel,-bwLevel]
        color2 = [bwLevel,bwLevel,colLevel]
#    
#    #lumimance test
#    bwLevel =0.95
#    colLevel = 0.5
        ...
        for p in range ((DSSW/2)*DSSH):
            if tCoords[p,4] ==1: # # if its an element
                 ...
                 if tCoords[p,9] == 1:
                    ...
                    e.setLineColor(color1,'rgb')
                 elif tCoords[p,9] == 0:
                    e.setFillColor(color2,'rgb')
                    e.setLineColor(color2,'rgb')
                 e.setPos([xPos,yPos])
                 dotcount = dotcount+1
                 e.draw()
...
#
#pattern(folds,'degraded',100,'BW',0,100) #This calls the experiment
#myWin.flip()
...
def DiscColor(colorCategory,responseScreenVersion):
    event.clearEvents()
    if responseScreenVersion == 1:
        responseScreen.setText('Color      NoColor')
        responseScreen.draw()
        myWin.flip()
        responseKey = event.waitKeys(keyList=['a', 'l']) 
        if responseKey == ['a']:
            choice = 'Color'
            if colorCategory == 'Col':
                respCorr = 1
            elif colorCategory == 'BW':
                respCorr = 0
        elif responseKey == ['l']:
            choice = 'NoColor'
            if colorCategory == 'Col':
        ...
        responseScreen.setText('NoColor       Color')
        responseScreen.draw()
        myWin.flip()
        responseKey = event.waitKeys(keyList=['a', 'l']) 
        if responseKey == ['l']:
...
def runBlock(trialbook,Reps):
    ...
    for thisTrial in trials:
        ...
        pattern(folds,reg,pRef,colorCategory,AS,100) #This calls the experiment
        ...
        background2.draw()
        background.draw()
        respCorr, choice = DiscColor(colorCategory, responseScreenVersion)
...
event.waitKeys(keyList = ['g'])
runBlock('trialbookDiscCol.xlsx',1)
message('HelloMain')
event.waitKeys(keyList = ['g'])
runBlock('trialbookDiscCol.xlsx',30)
message('Goodbye')
core.wait(2)
myWin.close
core.quit()
