classdef SPNCAT_Export_func < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function ExtractPlotData(app)
            ...
            if strcmp(app.TabGroup.SelectedTab.Title,'Full Timecourse')
                ...
                app.PLOT = app.CURRENTPROJECT(ismember([app.CURRENTPROJECT.SPNID],app.ConditionsListBox.Value));
                ...
                for iCond = 1:length(app.PLOT)
                    if app.DifferenceWaveButton.Value
                        app.PLOT(iCond).Plot_Data = app.PLOT(iCond).Data - app.PLOT(iCond).Data_Irregular;
                    else
                        app.PLOT(iCond).Plot_Data = app.PLOT(iCond).Data;
                    end
                    app.PLOT(iCond).Line_Data = squeeze(mean(mean(app.PLOT(iCond).Plot_Data(ElectrodeIndices,:,:),1),3));
                    app.PLOT(iCond).Mean = squeeze(mean(mean(app.PLOT(iCond).Plot_Data(ElectrodeIndices,TimeIndices,:,:),2),1));
                    app.PLOT(iCond).Legend_Name = [app.PLOT(iCond).Full_Name ' (SPN ' num2str(app.PLOT(iCond).SPNID) ')'];
                end
                ...
                app.LEGHANDLE = [];
                for iCond = 1:length(app.PLOT)
                    plot(app.UIAxesWaveform,app.TIME,app.PLOT(iCond).Line_Data);
                ...
                app.LEGHANDLE = legend(app.UIAxesLegend,strrep({app.PLOT.Legend_Name},'_',' '));
                ...
                [MinY_Cond, MaxY_Cond] = GetLimits(app,cat(1,app.PLOT.Line_Data));
        ...
        function string_BIDS = BIDS_String_Format(~,string)
            string_BIDS = lower(strrep(strrep(string,'_',''),' ',''));
        end
    ...
    methods (Access = private)
        ...
        function ExportMeanButtonPushed(app, event)
            ...
            app.EXPORT = app.CURRENTPROJECT(ismember([app.CURRENTPROJECT.SPNID],app.ConditionsListBox.Value));
            ...
            UniqueExperiments = unique({app.EXPORT.Full_Name});
            
            EXPORT_EXP = struct();
            
            for iExp = 1:length(UniqueExperiments)
                
                ExperimentIndices = find(strcmp({app.EXPORT.Full_Name},UniqueExperiments{iExp}));
                
                EXPORT_EXP(iExp).SPNID = [app.EXPORT(ExperimentIndices).SPNID];
                EXPORT_EXP(iExp).Project = app.EXPORT(ExperimentIndices(1)).Project;
                EXPORT_EXP(iExp).Experiment = app.EXPORT(ExperimentIndices(1)).Experiment;
                EXPORT_EXP(iExp).Full_Name = app.EXPORT(ExperimentIndices(1)).Full_Name;
                EXPORT_EXP(iExp).N = [app.EXPORT(ExperimentIndices).N];
                EXPORT_EXP(iExp).Regular_Condition = {app.EXPORT(ExperimentIndices).Regular_Condition};
                EXPORT_EXP(iExp).Irregular_Condition = {app.EXPORT(ExperimentIndices).Irregular_Condition};
                EXPORT_EXP(iExp).Data = {app.EXPORT(ExperimentIndices).Data};
                EXPORT_EXP(iExp).Data_Irregular = {app.EXPORT(ExperimentIndices).Data_Irregular};
                EXPORT_EXP(iExp).Time_Vector = {app.EXPORT(ExperimentIndices).TimeVector};
                ...
                EXPORT_EXP(iExp).Export_Data = array2table(EXPORT_EXP(iExp).Export_Data,'VariableNames',strrep({app.EXPORT(ExperimentIndices).Regular_Condition},' ','_'));
                
                UniqueRandomConditions = unique({app.EXPORT(ExperimentIndices).Irregular_Condition});
                RemovalIndex = ones(1,length({app.EXPORT(ExperimentIndices).Irregular_Condition}));
                for iCond = 1:length(UniqueRandomConditions)
                    ConditionIndex = strcmp({app.EXPORT(ExperimentIndices).Irregular_Condition},UniqueRandomConditions{iCond});
            ...
            Save_Long = [SavePath filesep 'P' nDigitString(app.EXPORT(1).Project,3) '_LONG.csv'];
            
            Count = 0; AlreadyExist = 0;
            while exist(Save_Long,'file')
                Count = Count + 1;
                Save_Long = [SavePath filesep 'P' nDigitString(app.EXPORT(1).Project,3) '_LONG-' num2str(Count) '.csv'];
                AlreadyExist = 1;
            end
            
            if AlreadyExist
                disp(['Long data for P' nDigitString(app.EXPORT(1).Project,3) ' already present, appended with -' num2str(Count) '.'])
            end
        ...
        function TabGroupSelectionChanged(app, event)
            ...
            if strcmp(selectedTab.Title,'Full Timecourse')
                ...
                app.ExportFormatButtonGroup.SelectedObject = app.ExportFormatButtonGroup.Buttons(1);
                
                % If alldata folders are not present, we disable them.
                
                FolderExists = 0;
                for iProject = 1:unique([app.CAT.Project])
                    if exist([app.CallingApp.DATADIR filesep 'Project ' num2str(iProject) 'alldata' filesep], 'dir')
                        FolderExists = 1;
                    end
                end
                ...
                if ~FolderExists
                    ...
                    app.BIDButton.Enable = 0;
                else
                    app.SETButton.Enable = 1;
                    app.BIDButton.Enable = 1;
                end
        ...
        function ExportTimecourseButtonPushed(app, event)
            ...
            if strcmp(app.ExportFormatButtonGroup.SelectedObject.Text,'.MAT') || strcmp(app.ExportFormatButtonGroup.SelectedObject.Text,'.AVR')
                ...
                EXPORT_TIMECOURSE = cell(length(Conditions_To_Export),1);
                Count = 0;
                for iCond = Conditions_To_Export
                    Count = Count + 1;
                    [EXPORT_TIMECOURSE{Count}, ~] = ExtractConditionData(app.CallingApp.CAT,app.CallingApp.DATADIR,iCond,0);
            ...
            elseif strcmp(app.ExportFormatButtonGroup.SelectedObject.Text,'.SET')
                ...
                for iCond = Conditions_To_Export
                    Count = Count + 1;
                    [EXPORT_TIMECOURSE{Count}, ~] = ExtractConditionData(app.CallingApp.CAT,app.CallingApp.DATADIR,iCond,1);
                    ProgressBar.Message = ['Getting data ready for export: SPN ' nDigitString(iCond,4)];
                end
                ...
                for iExport = 1:length(EXPORT_TIMECOURSE)
                    ...
                    if ~exist(SAVE_DIRECTORY_EXPERIMENT,'dir'); mkdir(SAVE_DIRECTORY_EXPERIMENT); end
            ...
            elseif strcmp(app.ExportFormatButtonGroup.SelectedObject.Text,'BIDS')
                ...
                for iExp = 1:length(Experiments_To_Export)
                    
                    Conditions_To_Export = [app.CAT(ismember({app.CAT.Full_Name},Experiments_To_Export{iExp})).SPNID];
                    EXPORT_TIMECOURSE = cell(length(Conditions_To_Export),1);
                    Error = zeros(length(Conditions_To_Export),1);
                    Count = 0;
                    for iCond = Conditions_To_Export
                        Count = Count + 1;
                    ...
                    Experiment_Indices = find(strcmp({EXPORT_TIMECOURSE.Full_Name},Experiments_To_Export{iExp}));
                    EXPORT_TIMECOURSE_EXP.SPNID = [EXPORT_TIMECOURSE(Experiment_Indices).SPNID];
                    EXPORT_TIMECOURSE_EXP.Project = EXPORT_TIMECOURSE(Experiment_Indices(1)).Project;
                    EXPORT_TIMECOURSE_EXP.Experiment = EXPORT_TIMECOURSE(Experiment_Indices(1)).Experiment;
                    EXPORT_TIMECOURSE_EXP.Full_Name = EXPORT_TIMECOURSE(Experiment_Indices(1)).Full_Name;
                    EXPORT_TIMECOURSE_EXP.SubjectNumbers_AllData = EXPORT_TIMECOURSE(Experiment_Indices(1)).SubjectNumbers_AllData;
                    EXPORT_TIMECOURSE_EXP.Task_Names = {EXPORT_TIMECOURSE(Experiment_Indices).Task_Names};
                    EXPORT_TIMECOURSE_EXP.AKA = {EXPORT_TIMECOURSE(Experiment_Indices).AKA};
                    EXPORT_TIMECOURSE_EXP.File_Notes = {EXPORT_TIMECOURSE(Experiment_Indices).File_Notes};
                    EXPORT_TIMECOURSE_EXP.Error = [EXPORT_TIMECOURSE(Experiment_Indices).Error];
                    if any(EXPORT_TIMECOURSE_EXP.Error)
                        continue
                    end
                    EXPORT_TIMECOURSE_EXP.Trigger_Info_Files = EXPORT_TIMECOURSE(Experiment_Indices(1)).SubjectFiles_Trigger_Info_AllData;
                    EXPORT_TIMECOURSE_EXP.Trigger_Info = load(fullfile(EXPORT_TIMECOURSE_EXP.Trigger_Info_Files.folder,EXPORT_TIMECOURSE_EXP.Trigger_Info_Files.name));
                    EXPORT_TIMECOURSE_EXP.Trigger_Info = EXPORT_TIMECOURSE_EXP.Trigger_Info.triggerInfo;
                    ...
                    for iSub = 1:length(EXPORT_TIMECOURSE_EXP.SubjectNumbers_AllData)
                        if ~isempty(BDF_Files)
                            EXPORT_TIMECOURSE_EXP.BDF_Files{iSub} = vertcat(BDF_Files{:,iSub});
                            [~, idx] = unique(strcat({EXPORT_TIMECOURSE_EXP.BDF_Files{iSub}.folder}',repelem({filesep},length(EXPORT_TIMECOURSE_EXP.BDF_Files{iSub}),1),{EXPORT_TIMECOURSE_EXP.BDF_Files{iSub}.name}'),'stable');
                            EXPORT_TIMECOURSE_EXP.BDF_Files{iSub} = EXPORT_TIMECOURSE_EXP.BDF_Files{iSub}(idx);
                            EXPORT_TIMECOURSE_EXP.BDF_Task_Indices{iSub} = idx;
                        end
                        if ~isempty(SET_Files)
                            EXPORT_TIMECOURSE_EXP.SET_Files{iSub} = vertcat(SET_Files{:,iSub});
                            [~, idx] = unique(strcat({EXPORT_TIMECOURSE_EXP.SET_Files{iSub}.folder}',repelem({filesep},length(EXPORT_TIMECOURSE_EXP.SET_Files{iSub}),1),{EXPORT_TIMECOURSE_EXP.SET_Files{iSub}.name}'),'stable');
                            EXPORT_TIMECOURSE_EXP.SET_Files{iSub} = EXPORT_TIMECOURSE_EXP.SET_Files{iSub}(idx);
                            EXPORT_TIMECOURSE_EXP.SET_Task_Indices{iSub} = idx;
                        end
                        if ~isempty(SET_ICA_Files)
                            EXPORT_TIMECOURSE_EXP.SET_ICA_Files{iSub} = vertcat(SET_ICA_Files{:,iSub});
                            [~, idx] = unique(strcat({EXPORT_TIMECOURSE_EXP.SET_ICA_Files{iSub}.folder}',repelem({filesep},length(EXPORT_TIMECOURSE_EXP.SET_ICA_Files{iSub}),1),{EXPORT_TIMECOURSE_EXP.SET_ICA_Files{iSub}.name}'),'stable');
                            EXPORT_TIMECOURSE_EXP.SET_ICA_Files{iSub} = EXPORT_TIMECOURSE_EXP.SET_ICA_Files{iSub}(idx);
                            EXPORT_TIMECOURSE_EXP.SET_ICA_Task_Indices{iSub} = idx;
                        end
                        if ~isempty(SET_ICA_Max_Files)
                            EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files{iSub} = vertcat(SET_ICA_Max_Files{:,iSub});
                            [~, idx] = unique(strcat({EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files{iSub}.folder}',repelem({filesep},length(EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files{iSub}),1),{EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files{iSub}.name}'),'stable');
                            EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files{iSub} = EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files{iSub}(idx);
                            EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Task_Indices{iSub} = idx;
                        end
                    end
                    ...
                    warning off;
                    BIDS_APPENDIX = readtable('BIDS.csv');
                    warning on;
                    
                    BIDS_ERROR = struct(); jCounter = 0;
                    
                    ProgressBar.Message = ['Exporting BIDS for ' EXPORT_TIMECOURSE_EXP.Full_Name];
                    
                    try
                        
                        if any(EXPORT_TIMECOURSE_EXP.Error)
                            error(['Error when checking for files for ' EXPORT_TIMECOURSE_EXP.Full_Name])
                        end
                        
                        BIDS = struct; BIDS_EXPORT = struct();
                        ...
                        switch app.DataFileforExportButtonGroup.SelectedObject.Text
                            ...
                            case 'BDF (Continuous)'
                                BIDS_DATA = EXPORT_TIMECOURSE_EXP.BDF_Files;
                                Task_Indices = EXPORT_TIMECOURSE_EXP.BDF_Task_Indices;
                            case 'SET (Epoched)'
                                BIDS_DATA = EXPORT_TIMECOURSE_EXP.SET_Files;
                                Task_Indices = EXPORT_TIMECOURSE_EXP.SET_Task_Indices;
                            case 'SET (Epoched, ICA)'
                                BIDS_DATA = EXPORT_TIMECOURSE_EXP.SET_ICA_Files;
                                Task_Indices = EXPORT_TIMECOURSE_EXP.SET_ICA_Task_Indices;
                            case 'SET (Epoched, ICA, Max)'
                                BIDS_DATA = EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Files;
                                Task_Indices = EXPORT_TIMECOURSE_EXP.SET_ICA_Max_Task_Indices;
                        end
                        ...
                        for iSub = 1:length(BIDS_DATA)
                            ...
                            if length(Data_File) == 1
                                ...
                                % If only one file is found for a subject,
                                % this will run.
                                
                                BIDS.DATA(iSub).file = [Data_File.folder filesep Data_File.name];
                                BIDS.DATA(iSub).session = 1;
                                BIDS.DATA(iSub).run = 1;
                                
                                if ~any(cellfun(@isempty,EXPORT_TIMECOURSE_EXP.Task_Names))
                                    BIDS.DATA(iSub).task{1} = BIDS_String_Format(app,EXPORT_TIMECOURSE_EXP.Task_Names{1});
                                else
                                    BIDS.DATA(iSub).task{1} = BIDS_String_Format(app,EXPORT_TIMECOURSE_EXP.AKA{1});
                                end
                                
                            elseif strcmp(app.DataFileforExportButtonGroup.SelectedObject.Text,'BDF (Continuous)') && any(cellfun(@isempty,EXPORT_TIMECOURSE_EXP.Task_Names))
                                
                                % If more than one file is found, the files
                                % are BDF's, and there are no task names,
                                % this will run. This will merge across the
                                % BDF files to make one.
                                
                                ALLEEG = []; CURRENTSET = 0;
                                for iFile = 1:length(Data_File)
                                    EEG = pop_biosig([Data_File(iFile).folder filesep Data_File(iFile).name], 'channels',[],'ref',48); % import raw data with CZ reference
                                    [ALLEEG, ~, CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET,'gui','off');
                                end
                                ...
                                for iRow = 1:length(EEG_MERGED.event)
                                    EEG_MERGED.event(iRow).type = str2double(EEG_MERGED.event(iRow).type);
                                end
                                ...
                                % If more than one file is found, and
                                % there are task names in the APPENDIX to
                                % indicate multiple tasks, this will run.
                                
                                if isempty(EXPORT_TIMECOURSE_EXP.Task_Names)
                                    error(['>1 ' app.DataFileforExportButtonGroup.SelectedObject.Text ' files present, but no "Task_Names" or "File_Notes" in APPENDIX to indicate this is expected, for ' EXPORT_TIMECOURSE_EXP.Full_Name ' - ' Data_File.name]);
                                end
                                ...
                                for iFile = 1:length(Data_File)
                                    ...
                                    BIDS.DATA(iSub).run(iFile) = 1;
                                    
                                    if ~isempty(EXPORT_TIMECOURSE_EXP.Task_Names{iFile})
                                        BIDS.DATA(iSub).task{iFile} = BIDS_String_Format(app,EXPORT_TIMECOURSE_EXP.Task_Names{Task_Indices{iSub}(iFile)});
                                    end
                                    
                                    % % %                                     if ~isempty(EXPORT_TIMECOURSE_EXP.File_Notes{iFile})
                                    % % %                                         BIDS.DATA(iSub).notes{iFile} = EXPORT_TIMECOURSE_EXP.File_Notes{iFile};
                                    % % %                                     end
                        ...
                        Experiment_Index = find(ismember(BIDS_APPENDIX.SPNID,EXPORT_TIMECOURSE_EXP.SPNID));
                        ...
                        for iEvent = 1:size(EXPORT_TIMECOURSE_EXP.Trigger_Info,1)
                            for iTrigger = 1:length(EXPORT_TIMECOURSE_EXP.Trigger_Info{iEvent,2})
                                BIDS.EVENTS_DES.Column{end+1,1} = 'TriggerValue';
                                BIDS.EVENTS_DES.Title{end,1} = 'Levels';
                                BIDS.EVENTS_DES.LevelName{end,1} = ['x' num2str(EXPORT_TIMECOURSE_EXP.Trigger_Info{iEvent,2}(iTrigger))];
                                BIDS.EVENTS_DES.Content{end,1} = EXPORT_TIMECOURSE_EXP.Trigger_Info{iEvent,1};
                            end
                        end
                        ...
                        %                         if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP(iExp).Full_Name filesep 'Participant_Info.csv'])
                        
                        %                             BIDS.PARTICIPANT_INFO = readtable(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP(iExp).Full_Name filesep 'Participant_Info.csv']);
                        
                        %                         else
                        
                        %                             disp(['Participant_Info.csv not found for ' EXPORT_TIMECOURSE_EXP(iExp).Full_Name ' - Using Default.']);
                        
                        BIDS.PARTICIPANT_INFO = table({''},{''},{''},{''},{''},'VariableNames',{'participant_id' 'species' 'age' 'sex' 'handedness'});
                        
                        for iSub = 1:length(BIDS_DATA)
                            BIDS.PARTICIPANT_INFO.participant_id{iSub,1} = ['sub-' nDigitString(iSub,3)];
                            BIDS.PARTICIPANT_INFO.species{iSub,1} = 'n/a';
                            BIDS.PARTICIPANT_INFO.age{iSub,1} = 'n/a';
                            BIDS.PARTICIPANT_INFO.sex{iSub,1} = 'n/a';
                            BIDS.PARTICIPANT_INFO.handedness{iSub,1} = 'n/a';
                        end
                        ...
                        BIDS.PARTICIPANT_INFO_DES.Column{1,1} = 'participant_id';
                        ...
                        BIDS.PARTICIPANT_INFO_DES.Column{2,1} = 'species';
                        BIDS.PARTICIPANT_INFO_DES.Title{2,1} = 'Description';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{2,1} = {};
                        BIDS.PARTICIPANT_INFO_DES.Content{2,1} = 'Species of subjects';
                        
                        BIDS.PARTICIPANT_INFO_DES.Column{3,1} = 'age';
                        BIDS.PARTICIPANT_INFO_DES.Title{3,1} = 'Description';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{3,1} = {};
                        BIDS.PARTICIPANT_INFO_DES.Content{3,1} = 'Age of subjects';
                        
                        BIDS.PARTICIPANT_INFO_DES.Column{4,1} = 'age';
                        BIDS.PARTICIPANT_INFO_DES.Title{4,1} = 'Units';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{4,1} = {};
                        BIDS.PARTICIPANT_INFO_DES.Content{4,1} = 'Years';
                        
                        BIDS.PARTICIPANT_INFO_DES.Column{5,1} = 'sex';
                        BIDS.PARTICIPANT_INFO_DES.Title{5,1} = 'Description';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{5,1} = {};
                        BIDS.PARTICIPANT_INFO_DES.Content{5,1} = 'Sex of subjects';
                        
                        BIDS.PARTICIPANT_INFO_DES.Column{6,1} = 'sex';
                        BIDS.PARTICIPANT_INFO_DES.Title{6,1} = 'Levels';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{6,1} = {'M' 'F'};
                        BIDS.PARTICIPANT_INFO_DES.Content{6,1} = {'Male' 'Female'};
                        
                        BIDS.PARTICIPANT_INFO_DES.Column{7,1} = 'handedness';
                        BIDS.PARTICIPANT_INFO_DES.Title{7,1} = 'Description';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{7,1} = {};
                        BIDS.PARTICIPANT_INFO_DES.Content{7,1} = 'Subjects dominant hand';
                        
                        BIDS.PARTICIPANT_INFO_DES.Column{8,1} = 'handedness';
                        BIDS.PARTICIPANT_INFO_DES.Title{8,1} = 'Levels';
                        BIDS.PARTICIPANT_INFO_DES.LevelName{8,1} = {'L' 'R'};
                        BIDS.PARTICIPANT_INFO_DES.Content{8,1} = {'Left' 'Right'};
                        ...
                        for iRow = 1:height(BIDS.PARTICIPANT_INFO_DES)
                            if isempty(BIDS.PARTICIPANT_INFO_DES.LevelName{iRow,1})
                                ...
                                for iLevel = 1:length(BIDS.PARTICIPANT_INFO_DES.LevelName{iRow,1})
                                    BIDS_EXPORT.PARTICIPANT_INFO_DES.(BIDS.PARTICIPANT_INFO_DES.Column{iRow,1}).(BIDS.PARTICIPANT_INFO_DES.Title{iRow,1}).(BIDS.PARTICIPANT_INFO_DES.LevelName{iRow,1}{iLevel}) = BIDS.PARTICIPANT_INFO_DES.Content{iRow,1}{iLevel};
                                end
                            end
                        end
                        ...
                        % =========================================================== %
                        % Prepare BIDS (TASK INFO).
                        % =========================================================== %
                        
                        if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Task_Info.csv'], 'file')
                            
                            BIDS.TASK_INFO = readcell(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Task_Info.csv']);
                            
                            for iRow = 1:size(BIDS.TASK_INFO,1)
                                BIDS_EXPORT.TASK_INFO.(BIDS.TASK_INFO{iRow,1}) = BIDS.TASK_INFO{iRow,2};
                            end
                            
                            if contains(app.DataFileforExportButtonGroup.SelectedObject.Text, 'SET')
                                disp('Loading example dataset to get parameters (e.g., sampling rate).')
                                if iscell(BIDS_EXPORT.DATA(1).file)
                                    EEG = pop_loadset(BIDS_EXPORT.DATA(1).file{1});
                                else
                                    EEG = pop_loadset(BIDS_EXPORT.DATA(1).file);
                                end
                                BIDS_EXPORT.TASK_INFO.SamplingFrequency = EEG.srate;
                                BIDS_EXPORT.TASK_INFO.EOGChannelCount = 0;
                            end
                            
                        else
                            
                            disp(['Task_Info.csv not found for ' EXPORT_TIMECOURSE_EXP.Full_Name ' - Using Default.']);
                            
                            BIDS_EXPORT.TASK_INFO = [];
                            
                        end
                        
                        if isempty(EXPORT_TIMECOURSE_EXP.Task_Names)
                            BIDS_EXPORT.TASK_INFO.TaskName = lower(strrep(strrep(EXPORT_TIMECOURSE_EXP.AKA{1},'_',''),' ',''));
                        else
                            BIDS_EXPORT.TASK_INFO.TaskName = EXPORT_TIMECOURSE_EXP.Task_Names;
                            for iTask = 1:length(BIDS_EXPORT.TASK_INFO.TaskName)
                                BIDS_EXPORT.TASK_INFO.TaskName{iTask} = lower(strrep(strrep(BIDS_EXPORT.TASK_INFO.TaskName{iTask},'_',''),' ',''));
                            end
                        end
                        
                        % =========================================================== %
                        % Prepare BIDS (CHANNEL INFO).
                        % =========================================================== %
                        
                        % % %                         if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP(iExp).Full_Name filesep 'Channel_Info.csv'])
                        % % %
                        % % %                             BIDS.CHANNEL_INFO = readtable(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP(iExp).Full_Name filesep 'Channel_Info.csv']);
                        % % %
                        % % %                             Headers = BIDS.CHANNEL_INFO.Properties.VariableNames;
                        % % %                             BIDS_EXPORT.CHANNEL_INFO = [Headers; table2cell(BIDS.CHANNEL_INFO)];
                        % % %
                        % % %                         else
                        % % %
                        % % %                             disp(['Channel_Info.csv not found for ' EXPORT_TIMECOURSE_EXP(iExp).Full_Name ' - Using Default.']);
                        % % %
                        % % %                             BIDS_EXPORT.CHANNEL_INFO = [];
                        % % %
                        % % %                         end
                        
                        %                         if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Channel_Info.csv'])
                        
                        BIDS_EXPORT.CHANLOCS = {};
                        for iSub = 1:length(EXPORT_TIMECOURSE_EXP.SubjectNumbers_AllData)
                            BIDS_EXPORT.CHANLOCS{iSub} = table2struct(readtable(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'ChanLocs' filesep EXPORT_TIMECOURSE_EXP.Full_Name '_S' nDigitString(EXPORT_TIMECOURSE_EXP.SubjectNumbers_AllData(iSub),2) '_ChanLocs.csv']));
                            
                            if contains(app.DataFileforExportButtonGroup.SelectedObject.Text, 'SET')
                                BIDS_EXPORT.CHANLOCS{iSub}(strcmp({BIDS_EXPORT.CHANLOCS{iSub}.type},'EOG')) = [];
                            end
                        ...
                        %                         else
                        %
                        %                             error(['Channel_Info.csv not found for ' EXPORT_TIMECOURSE_EXP.Full_Name ' - Required to Export.']);
                        %
                        %                         end
                        
                        % =========================================================== %
                        % Prepare BIDS (STIMULI).
                        % =========================================================== %
                        
                        BIDS_EXPORT.STIMULI = {};
                        % % %                         if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Stimuli' filesep])
                        % % %                             BIDS.STIMULI = dir(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Stimuli' filesep '*']);
                        % % %                             BIDS.STIMULI(1:2) = [];
                        % % %                             for iStim = 1:length(BIDS.STIMULI)
                        % % %                                 BIDS_EXPORT.STIMULI{iStim} = [BIDS.STIMULI(iStim).folder filesep BIDS.STIMULI(iStim).name];
                        % % %                             end
                        % % %                         else
                        % % %                             disp(['No stimuli folder found for ' EXPORT_TIMECOURSE_EXP.Full_Name ' - no stimuli will be included!'])
                        % % %                         end
                        ...
                        BIDS_EXPORT.CODE = {};
                        if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Code' filesep], 'dir')
                            BIDS.CODE = dir(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'Code' filesep '*']);
                            BIDS.CODE(1:2) = [];
                            for iCode = 1:length(BIDS.CODE)
                                BIDS_EXPORT.CODE{iCode} = [BIDS.CODE(iCode).folder filesep BIDS.CODE(iCode).name];
                            end
                        else
                            disp(['No code folder found for ' EXPORT_TIMECOURSE_EXP.Full_Name ' - no code will be included!'])
                        end
                        ...
                        if exist(['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'README'], 'file')
                            BIDS_EXPORT.README = ['BIDS_resources' filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep 'README'];
                        else
                            BIDS_EXPORT.README = [];
                        end
                        ...
                        Save_Directory_Exp = [Save_Directory filesep EXPORT_TIMECOURSE_EXP.Full_Name filesep];
                        if ~exist(Save_Directory_Exp, 'dir'); mkdir(Save_Directory_Exp); end
                        ...
                        bids_export(BIDS_EXPORT.DATA, ...
                            ...
                            'tInfo', BIDS_EXPORT.TASK_INFO, ...
                            'chanlocs', BIDS_EXPORT.CHANLOCS, ...
                            'stimuli', BIDS_EXPORT.STIMULI, ...
                            'codefiles', BIDS_EXPORT.CODE, ...
                            'README', BIDS_EXPORT.README);
                        ...
                        jCounter = jCounter + 1;
                        BIDS_ERROR(jCounter).Full_Name = EXPORT_TIMECOURSE_EXP.Full_Name;
        ...
        function FilterValueChanged(app, event)
            ...
            if app.MinWEditField.Value == 0
                Indices_W = true(1,length([app.CAT.W]));
            else
            ...
            elseif app.IgnoreRegularityCheckBox.Value
                ...
                Indices_AttendReg = true(ones(1,length([app.CAT.W])));
            end
            
            Indices_AllData = true(1,length(app.CAT));
            if strcmp(app.ExportFormatButtonGroup.SelectedObject.Text, '.SET')
                for iProject = 1:length(app.CAT)
                    if ~exist([app.CallingApp.DATADIR filesep 'Project ' num2str(app.CAT(iProject).Project) 'alldata' filesep], 'dir')
                        Indices_AllData(iProject) = false;
                    end
                end
            end
            
            FilterIndices = Indices_SPN & Indices_D & Indices_W & Indices_AttendReg & Indices_AllData;
        ...
        function ExportFormatButtonGroupSelectionChanged(app, event)
            ...
            elseif strcmp(selectedButton.Text,'BIDS')
                ...
                Experiment_Indices = true(1,length(app.EXP));
                for iExp = 1:length(app.EXP)
                    if ~exist([app.CallingApp.DATADIR filesep 'Project ' num2str(app.EXP(iExp).Project) 'alldata' filesep], 'dir')
                        Experiment_Indices(iExp) = false;
                    end
                end
                
                app.ProjectsListBox.Items = {app.EXP(Experiment_Indices).Full_Name};
